-- +goose Up
-- Baseline: the schema as it was when migrations were still applied by the
-- startup routine in Go. Every statement is idempotent, so an existing
-- development database adopts goose without errors and a fresh one is built
-- from scratch. Do not edit; add new migrations instead.
...
-- +goose Down
-- The baseline cannot be rolled back: restore from a backup instead (RNF-014).
