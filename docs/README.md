...
- *Próximas fatias, em ordem sugerida:* criação de conta pelo admin, exclusão de conta separada do bloqueio, pedidos de redefinição de senha aprovados por owner ou admin (DEC-063), transferência de titularidade em duas etapas com comando local de recuperação (DEC-057, DEC-058), e por fim o LDAP no mesmo formulário de login (DEC-072 a 075).
...
O worker: `cd worker && venv/bin/python -m pytest` (82 testes). O frontend: `cd frontend && npm test` (77 testes). `make test` executa as três suítes e agora propaga falhas. Não use o banco de desenvolvimento como `TEST_DATABASE_URL`. Sem essa variável, os testes Go de integração continuam sendo ignorados: sucesso de `make test` sozinho não comprova integração.
