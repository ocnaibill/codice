...
- *Roteiro para o Authentik (a conferir na documentação do Authentik, pois não foi testado):* crie um provedor **LDAP** e um *outpost* LDAP; crie uma conta de serviço para o Códice com permissão de buscar no diretório; use o DN dessa conta como `LDAP_BIND_DN`. Pelo que sei, no Authentik `cn` é o nome de usuário e `uid` é um identificador estável, o que sugere `LDAP_USER_FILTER=(cn={username})`, `LDAP_USERNAME_ATTR=cn`, `LDAP_ID_ATTR=uid` e `LDAP_EMAIL_ATTR=mail`, com a base do provedor (algo como `dc=ldap,dc=goauthentik,dc=io`). O outpost oferece LDAPS e StartTLS; se o certificado for de uma CA própria, aponte `LDAP_CA_FILE`. Confirme cada atributo com uma busca real antes de ligar.
