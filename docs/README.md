...
- *Próximas fatias, em ordem sugerida:* o LDAP no mesmo formulário de login, com roteamento pela conta e o dono sempre local (DEC-072 a 075). Antes dele vale revisar o pacote de contas inteiro, já que ele mexe no login.
...
O worker: `cd worker && venv/bin/python -m pytest` (82 testes). O frontend: `cd frontend && npm test` (109 testes). `make test` executa as três suítes e agora propaga falhas. Não use o banco de desenvolvimento como `TEST_DATABASE_URL`. Sem essa variável, os testes Go de integração continuam sendo ignorados: sucesso de `make test` sozinho não comprova integração.
