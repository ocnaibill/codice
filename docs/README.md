...
- *Próximas fatias, em ordem sugerida:* transferência de titularidade em duas etapas com comando local de recuperação, incluindo a redefinição do dono (DEC-057, DEC-058), e por fim o LDAP no mesmo formulário de login (DEC-072 a 075).
...
O worker: `cd worker && venv/bin/python -m pytest` (82 testes). O frontend: `cd frontend && npm test` (98 testes). `make test` executa as três suítes e agora propaga falhas. Não use o banco de desenvolvimento como `TEST_DATABASE_URL`. Sem essa variável, os testes Go de integração continuam sendo ignorados: sucesso de `make test` sozinho não comprova integração.
