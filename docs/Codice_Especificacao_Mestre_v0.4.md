...
| DEC-089 | (confirmada pelo mantenedor em 21/09/2026) O reforço semântico da retomada equivalente é uma **capacidade local opcional**, desligada por padrão na interface, com LaBSE como modelo inicial e contrato de provedor substituível | Na pilha completa, o worker de embeddings fica separado e ocioso até o owner ativar o recurso; na primeira ativação baixa o modelo automaticamente para cache persistente e a tela expõe disponibilidade, preparação, pronto ou erro. Desativar impede calcular e usar embeddings sem apagar o cache. O worker não bloqueia ingestão nem leitura e nenhum texto é enviado a terceiros. Cada vetor registra provedor, modelo, revisão, dimensão, normalização e versão do pré-processamento e só é comparado com vetores compatíveis. A semântica entra depois das evidências lexical e por âncoras, exige limiar mínimo e vizinho mais próximo recíproco. O LaBSE foi escolhido apesar do custo (cerca de 1,9 GB em disco, ~1 GB de RAM e ~14 trechos/s no M4) porque, no ensaio de prosa do Duna com nomes e dígitos ocultos, ficou a até um trecho em 93% de EN↔PT, 93% de PT-PT↔PT-BR, 91% de EN↔AR e 88% de PT↔AR; e5-small caiu para 52% e 34% nos pares com árabe. Vetores são derivados e ficam fora do backup |
