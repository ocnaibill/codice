...
| DEC-087 | (registrada com a implementação; a confirmar pelo mantenedor) Todo arquivo com sumário ganha uma **estrutura padrão** (partes, capítulos, e o que é frente, corpo ou fim do livro), e a retomada equivalente (RF-042) procura **capítulo primeiro, depois a passagem, depois confere o caminho de volta** | O worker lê o sumário do EPUB (NCX ou documento de navegação, com as âncoras que apontam para o meio de um arquivo) e os marcadores do PDF; cada trecho sabe o nó em que está e nunca cruza dois; cada nó é frente, corpo ou fim pelo que se chama e pelo lugar em que está (a palavra só vale quando abre o título, para "O fim do mundo" continuar sendo um capítulo). O motor corta a história de cada arquivo na profundidade em que os dois têm o mesmo número de divisões ("3 livros" com "3 livros", "28 capítulos" com "28"), confere os números dos títulos quando existem e trata a numeração que recomeça como outro livro no mesmo arquivo; a busca das palavras e dos nomes fica **dentro do capítulo alinhado** quando os números confirmam, corpo só procura corpo (um apêndice que cita todo mundo nunca disputa com um capítulo), e o que foi achado é conferido no caminho de volta (descartado, se apoiado só em nomes, ou rebaixado). Sem sumário nos dois lados vale o que valia (o arquivo como capítulo), com confiança limitada a média. Uma divisão que é mais de 10% da obra (um livro de três) nunca passa de média, porque a divisão certa não é o lugar perto. **Medido** (ensaio de 21/09/2026, sem modelo): no Verne (domínio público, três edições, verdade pelos capítulos), o capítulo certo em 214 de 219 ofertas entre idiomas (as 5 restantes são fronteira da régua; antes, 53% com metade das ofertas), e 6 de 224 tentativas sem resposta; no Duna, PT-PT com PT-BR passou de 5 para 19 achados e as âncoras ficaram todas na posição esperada. Limites: o teste só mede o capítulo, não o parágrafo; livros com divisões diferentes entre as edições ou sem sumário (PDF sem marcadores) continuam com o método antigo; os limiares seguem a ser medidos no conjunto de avaliação (#32, QA-028) |
| DEC-088 | (registrada com a implementação; a confirmar pelo mantenedor) Quando os dois arquivos têm capítulos alinhados e nenhuma passagem foi achada, a retomada equivalente oferece **uma posição aproximada dentro do capítulo** (a mesma fração do caminho, por caracteres), rotulada como aproximada, no lugar do começo do capítulo | O texto de um capítulo traduzido tem comprimento proporcional em todas as suas partes, então a mesma fração do capítulo do outro arquivo cai perto. Só existe dentro do capítulo que as estruturas confirmaram (nunca sobre a obra inteira), a precisão é `approximate` (a interface diz "posição aproximada no mesmo capítulo"), a confiança é média no máximo (só com números confirmando e capítulo pequeno) e baixa nos demais; uma passagem achada dentro do capítulo sempre a substitui. Isso muda a promessa de "nunca inventa" da DEC-086 num ponto: é uma estimativa declarada, não uma correspondência. **Medido:** em 118 pares de trechos do Verne, a posição proporcional dentro do capítulo cai a até 1 trecho em 98% dos casos (o começo do capítulo, 25%); ponta a ponta na API, 96% a até 1 trecho (159 tentativas). Numa unidade de ~250 trechos (um livro inteiro), a mesma estimativa cai a 7%: por isso a confiança cai para baixa e o ganho ali é do modelo opcional (#31) |
