...
**Estado em 20 de setembro de 2026: em andamento (`feat/fase5-leitura`).** Feitos e exercitados no navegador: locator versionado por formato e progresso por arquivo com revisão otimista, ficha da obra com escolha de edição, idioma e arquivo (DEC-028), retomada da posição própria do arquivo ou do começo, "Continuar lendo" pelo arquivo lido por último, notas com tipo, Markdown, tags e locator, busca, edição e exportação (RF-016, RF-017, FL-09), isolamento entre usuários. Os três testes do plano passam (troca de arquivo retoma a posição própria; a nota exporta com a referência sem a fonte; dois usuários não enxergam o estado um do outro). Decididos e implementados: tempo de leitura ativo (DEC-076), conclusão como fato do arquivo (DEC-077) e o modo dos quadrinhos lembrado entre quadrinhos (DEC-078). Também a versão que conta (a última aberta depois de começada), a conclusão com histórico, contador e "obra finalizada", e o botão "Ler" que abre a versão que conta ou a ficha (DEC-079 a DEC-081). Os leitores de EPUB (href, progressão, percentual) e de texto e Markdown (posição de rolagem) também gravam posição. Faltam as telas, registradas nas issues #13 (anotações) e #14 (sumário do PDF e falha de localização); a retomada equivalente (RF-042) é evolução e exige antes o texto extraído e guardado, não o OCR. **O texto guardado (issue #17, DEC-082 a DEC-085) e a retomada equivalente entre versões sobre ele (issue #18, DEC-086) estão feitos.**
...
**Estado em 19 de setembro de 2026: em andamento.** Feitos: Dockerfiles e Docker Compose completos com healthchecks e ordem de partida (§10.4), `GET /healthz` por componente com falha isolada do Redis (RF-021), confiança explícita em proxies para o limite de login e a documentação de implantação no README. Feito também o comando de backup e restauração com verificação, criptografia opcional, ensaio de restauração e retenção (RF-034, RNF-010, DEC-064), exercitado com uma perda total dos volumes. Feito também o healthcheck do worker por arquivo de batimento. Feito também o fechamento: o ensaio de restauração em projeto separado e a medição por etapa (base do RTO, com números sintéticos no README), a correção do backup que omitia arquivos ilegíveis em silêncio, o `scripts/backup.sh` e o perfil `ldap` do compose exercitado com um diretório LDAPS de teste. **Faltam, e só o mantenedor pode fazer:** medir o RTO com o acervo real e subir o contêiner oficial do Authentik dentro do compose.
