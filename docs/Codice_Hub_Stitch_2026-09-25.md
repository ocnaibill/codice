...
- Para usar o frontend do checkout contra essa pilha, `cd frontend && CODICE_DEV_API_TARGET=http://127.0.0.1:18091 npm run dev`. O Vite usa o proxy na mesma origem e, nesse modo, ignora uma eventual `VITE_API_URL` preexistente. Login e catálogo foram conferidos no navegador também por essa rota de desenvolvimento. Para receber eventos WebSocket pelo Vite, a origem dele também precisa estar em `CORS_ALLOWED_ORIGINS` do backend; a pilha Docker na porta 18091 já usa a mesma origem e não precisa desse ajuste.
